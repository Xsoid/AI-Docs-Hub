...
После первого onboarding или изменения MCP config перезапустите Codex. Полная модель использования и список read-only tools описаны в `docs/operations/codebase-memory.md`.
