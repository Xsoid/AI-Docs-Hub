...
В bundle не зашиваются абсолютные пути к checkout. При запуске приложение ищет корень хаба вверх от собственного `.app` и запускает:

```sh
scripts/hub-status --json
```
